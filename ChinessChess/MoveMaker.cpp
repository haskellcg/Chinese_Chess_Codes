...
bool MoveMaker::makeMove(const Move& _move)
{
	...
	//改变CChessStruct的成员
	if((cChess->isRed == true) && !chessIsNewForMe)
		++(cChess->roundsNumber);
...
bool MoveMaker::unMakeMove()
{
	...
	else
	{
		...
		RoundsCapture counterTemp;
		if(this->moveHistoryLen == 0)
		{
			counterTemp.movedRoundsNumber=counterTemp.movedCaptureStepNumber=0;
		}
		else
		{
			counterTemp=this->counterHistory[this->moveHistoryLen-1];
		}
		int toChessManValue=cChess->cChessArray[temp.to];
		cChess->cChessArray[temp.from]=toChessManValue;
		if(temp.capture!=0)
	...
	if(isChessManChinessName(_chinessMove.chessMan))
	{
		...
		if(isRedOnBigEnd && isRedChessMan)
		{
			...
			for(int i=3;i<=12;i++)
			{
				...
				if(tempChessManValue!=0 && tempChessManValue<32 && getChessManChinessName(tempChessManValue)==_chinessMove.chessMan)
				{
		...
		else if(!isRedChessMan && isRedOnBigEnd)
		{
			...
			{
				int tempChessManValue=_cChess->cChessArray[i*16+col];
				if(tempChessManValue!=0 && tempChessManValue>31 && getChessManChinessName(tempChessManValue)==_chinessMove.chessMan)
				{
					chessManValue=tempChessManValue;
					chessManPosition=i*16+col;
					break;
			...
			if(_chinessMove.act == UBString("平"))
			{
				int destCol=getNumber(_chinessMove.to,0)+2;
				toPosition=chessManPosition+destCol-col;
			}
			else if(_chinessMove.act == UBString("进"))
			{
				...
				{
					int off=getNumber(_chinessMove.to,0);
					toPosition+=off*16;
				}
		...
		else
		{
			...
			if(_chinessMove.act == UBString("平"))
			{
				int destCol=12-getNumber(_chinessMove.to,0);
				toPosition=chessManPosition+destCol-col;
			}
			else if(_chinessMove.act == UBString("进"))
			{
...
WXFMove MoveMaker::translateToWXF(SmartPtr<CChessStruct> _cChess,const Move& _move)
{
	...
	if(isChessManChinessName(_chinessMove.chessMan))
	{
		...
		if(getNumber(_chinessMove.to)==-1)
		{
